...
<div>
   ...
   <div class="flex justify-between items-center px-3 mb-3">
      <!-- btn craete -->
      <div>
         <a class="inline-block px-5 mr-auto border border-gray-300 rounded-lg hover:bg-blue-500 hover:shadow hover:text-white <?= $this->page == "create-category" ? "bg-blue-500  text-white shadow" : null ?>" href="<?= $this->request("{$this->view}|create-category") ?>">create</a>
      </div>

      <!-- search -->
      <form class="inline-block relative focus-within:text-gray-500" action="<?= $this->request("category") ?>" method="post">
   ...
   <div class="table text-center shadow-lg w-full">
      ...
      <div class="table-header-group bg-gray-300 text-lg font-semibold">
         ...
         <div class="table-cell py-2 px-5">Category</div>
         ...
         <?php if ( $categories ) : ?>      
            <?php foreach ( $categories as $category ) : ?>
               <div class="table-row">
                  ...
                  <div class="table-cell py-2 px-5"><?= $category['category'] ?></div>
                  <div class="table-cell py-2 px-5"><img class="h-10 w-10 mx-auto" src="<?= Image::show($this->view, $category['image']) ?>" alt="" height="40px" width="40px"></div>
                  <div class="table-cell py-2 px-5">
                     <!-- btn edit -->
                     <a class="inline-block px-5 border border-gray-300 rounded-lg hover:bg-yellow-500 hover:shadow hover:text-white <?= $this->name == $category['category'] ? "bg-yellow-500 text-white shadow" : null ?>" href="<?= $this->request("{$this->view}|edit-category|{$category['category']}") ?>">
                        edit
                     </a>

                     <!-- btn delete -->
                     <form class="inline" action="<?= $this->request($this->view) ?>" method="post">
                        <input type="hidden" name="action" value="dropCategory">
