   ...
   <div class="grid gap-3 xl:gap-6 grid-cols-2 xl:grid-cols-5 mt-6 xl:mt-13">
      <?php foreach ( $categories as $category ) : ?>         
         <div class="pb-5 overflow-hidden ">
            <a href="<?= $this->request("product|catalog|{$category['category']}") ?>" class="inline-block h-40 hover:shadow-xl">
               <img class="w-full h-full" src="<?= Image::storage($this->page, $category['image']) ?>" alt="<?= $category['category'] ?>">
            </a>
            <a href="<?= $this->request("product|catalog|{$category['category']}") ?>" class="block text-center text-lg font-semibold hover:underline"><?= $category['category'] ?></a>
         </div>   
      <?php endforeach ;?>
   </div>   
</main>
